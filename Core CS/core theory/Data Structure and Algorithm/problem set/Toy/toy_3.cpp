...
void logicLeft (int* T) {
	char temp;
	temp = T[0]; T[0] = T[1]; T[1] = T[2]; T[2] = T[3]; T[3] = temp;
	temp = T[7]; T[7] = T[6]; T[6] = T[5]; T[5] = T[4]; T[4] = temp;
	return;
}

void centerAnticlockwise (int* T) {
	char temp;
	temp = T[1]; T[1] = T[2]; T[2] = T[5]; T[5] = T[6]; T[6] = temp;
	return;
}
