...
template <typename T> KdTree<T>* buildKdTree (T* P, int d, int size) {
	...
	mid = (size - 1) / 2;
	root->splitLine = findMedian(root, P, 0, size-1);
...
template <typename T> int findMedian (KdTree<T>* root, T* P, int start, int end) {
	int left = start;
	int right = end - 1;

	if (root->splitDirection) {  // direction != 0, means y-axis
		if (start == end) {root->data = P[0]; return P[0].y; }
		...
		if (left > mid) findMedian(root, P, start, left);
		else if (left < mid) findMedian(root, P, left, end);
		else { root->data = P[mid]; return P[mid].y; }
	}
	else {  // means x-axis
		if (start == end) {root->data = P[0]; return P[0].x; }
		...
		else { root->data = P[mid]; return P[mid].x; }
	}
}
